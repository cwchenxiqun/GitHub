...
<head>
    ...
    <!--地区下拉选择插件-->
    <script type="text/javascript" src="plugins/cityselect/js/jquery.cityselect.js"></script>
...
<body onload="init();">
<form action="dealerUser/${msg }.do" name="userForm" id="userForm" method="post">
    ...
    <div id="zhongxin">
        <table>
            ...
            <c:if test="${fx != 'head'}">
                <tr>
                    <td><input type="text" name="DEALER_CODE" id="DEALER_CODE" value="${pd.DEALER_CODE }" maxlength="32" placeholder="这里输入激活码" title="激活码"/></td>
                </tr>
            </c:if>
            ...
            <tr>
                <td>
                    <div id="city" style="display: inline">
                        <select class="prov" style="width: 72px" style="display: inline" name="PROVINCE" id="PROVINCE" value="${pd.PROVINCE}"></select>
                        <select class="city" style="width: 72px" style="display: inline" disabled="disabled" name="CITY" id="CITY" value="${pd.CITY }"></select>
                        <select class="dist" style="width: 72px" style="display: inline" disabled="disabled" name="DISTRICT" id="DISTRICT" value="${pd.DISTRICT }"></select>
                    </div>
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
    ...
    var provinceInit = function () {
        if ("${pd.PROVINCE}" != "") {
            ...
            provinceSelect.value = "${pd.PROVINCE}";

            if (provinceSelect.fireEvent) {
                oButton.fireEvent("change");
            } else {
                var evt = document.createEvent('HTMLEvents');
                evt.initEvent('change', true, true);
                provinceSelect.dispatchEvent(evt);
            }
            setTimeout('cityInit();', 100);//初始化城市
        }
    }
    ...
    var cityInit = function () {
        if ("${pd.CITY}" != "") {
            ...
            citySelect.value = "${pd.CITY}";
            if (citySelect.fireEvent) {
                oButton.fireEvent("change");
            } else {
                var evt = document.createEvent('HTMLEvents');
                evt.initEvent('change', true, true);
            ...
            setTimeout('districtInit();', 100);//初始化地区
        }
    }
     ...
     * 初始化地区
     */
    var districtInit = function () {
        if ("${pd.DISTRICT}" != "") {
            document.getElementById("DISTRICT").value = "${pd.DISTRICT}";
        }
    }
    ...
    $(function () {
        ...
        $(function () {
            ...
            setTimeout('provinceInit();', 100);//初始化省份
        });
